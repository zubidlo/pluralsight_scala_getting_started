...
class FilterCheckerTests extends FlatSpec{
  ...
  "return a list with that file" in {
    val listOfFiles: List[IOObject] = List(FileObject(new File("random")), FileObject(new File("match")))
  ...
  "should not return the directory" in {
    val listOfIOObjects: List[IOObject] = List(FileObject(new File("random")), DirectoryObject(new File("match")))
